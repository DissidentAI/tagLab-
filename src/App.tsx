...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 selection:bg-indigo-500 selection:text-white font-sans antialiased flex flex-col">
      {/* Top Navbar */}
      <Navbar
        activeTab={activeTab}
      ...
      {selectedTags.length > 0 && !isBasketOpen && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 max-w-lg w-[calc(100%-2rem)]">
          <div className="bg-white/95 border border-indigo-200 rounded-2xl p-3 shadow-xl backdrop-blur-xl flex items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <div className="text-xs">
                <p className="font-bold text-slate-800">Tags sélectionnés</p>
                <p className="text-slate-500 text-[11px]">Prêts à être copiés ou exportés</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs flex items-center gap-1.5 shadow-sm transition-all"
              >
      ...
      <TagBasketDrawer
        ...
        artistName={artistName}
        trackName={trackName}
        onCopySuccess={(text, title) => showToast(title, `(${text.length} caractères)`, 'success')}
      />
